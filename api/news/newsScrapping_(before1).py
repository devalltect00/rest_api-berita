...
class SourceNews:
    ...
    def getAvailableNewsSourceByTag(self):
        try:
            ...
            for sourceNews, id in availableNewsByTag.items():
                ...
                # link = self.retrieveModifiedLink(link)[0]
                links = getLinkBasedOnNearMonth(link)
                listNews.append({
                    "name":name,
                    "link":links
                })
    ...
    def getAvailableNewsSourceByTagBasedId(self, idTarget):
        try:
            for sourceNews, id in availableNewsByTag.items():
                if idTarget == id:
                    name = self.source[id-1]["name"]
    ...
    def retrieveModifiedLink(self, link):
        ...
        currentMonth = translatorEnglishToIndonesian.translate(currentMonth)
...
## get the link for previous, current, and next month
def getLinkBasedOnNearMonth(link):
    scope = SCOPE_NEWS.INTERNATIONAL.value
    # currentYear = str(datetime.now().year)
    currentYear = str(calendar.datetime.datetime.now().year)
    # currentMonth = str(datetime.now().month)

    nowMonthInt = calendar.datetime.datetime.now().month

    ## previous month
    previousMonthInt = nowMonthInt - 1
    if previousMonthInt == 0:
        previousMonthInt = 12
        currentYear -= 1
    
    ## next month
    nextMonthInt = nowMonthInt + 1
    if nextMonthInt == 13:
        nextMonthInt = 1
        currentYear += 1

    monthList = [getMonth(previousMonthInt),
             getMonth(nowMonthInt),
             getMonth(nextMonthInt)]
    ...
    for month in monthList:
        result.append(link.replace("cakupan",scope).replace("bulan",month).replace("tahun",currentYear))
    
    return result

def getMonth(monthOnIntFormat):
    currentMonth = str(calendar.month_name[monthOnIntFormat])
    currentMonth = translatorEnglishToIndonesian.translate(currentMonth)
    return currentMonth

class NewsChannelByUrl(SourceNews):
